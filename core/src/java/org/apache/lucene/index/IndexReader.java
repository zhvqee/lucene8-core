 ...
 *
 *  一个索引快照
 *  非常有用的一个类型 DirectoryReader.open(IndexWriter) 获取一个IndexReader
 *
 *  想知道这个索引 已经改变的内容，DirectoryReader#openIfChanged
 *
 *  LeafReader : 叶子读取器，用于检索 stored fields, doc values, terms,
 *   and postings.
 *   CompositeReader :组合读取器，CompositeReader#getSequentialSubReaders 接着取检索
 *
 *   怎么获取这个IndexReader: DirectoryReader.open() 从磁盘获取数据
 *   DirectoryReader 也是个IndexReader,但是呢，他的父类是一个CompositeReader, 所以他不能直接获取一个倒排索引
 *
 *
 *
...
public abstract class IndexReader implements Closeable {
  ...
  /**
   *  返回 IndexReader 的 root Context ==> CompositeReaderContext
   *   非 CompositeReader ，返回LeafReaderContext
   *
   *
   * Expert: Returns the root {@link IndexReaderContext} for this
   * {@link IndexReader}'s sub-reader tree. 
   * <p>
   * If this reader is composed of sub
  ...
  /**
   *  返回字段field 有多少个分词下的文档相加 所有的分词累加 docFreq()
