...
/**
 *   DirectoryReader 实现继承了 CompositeReader, 且是一个抽象类
 *   一般通过静态方法open 获取一个DirectoryReader,
 *   默认通过StandardDirectoryReader
 *
...
public abstract class DirectoryReader extends BaseCompositeReader<LeafReader> {
  ...
  /**
   *  返回所有在目录Directory下的所有commit,
   *  一般策略是 KeepOnlyLastCommitDeletionPolicy
   *   该目录下，至少有一个IndexCommit ,不然异常
